...
@Component
public class InsertRoleTitleExecutor extends BaseExecutor<Object> {
    @Autowired
    ...
    public Object exectueBusiness(HttpServletRequest request,BusinessModel businessModel,Object... obj) {
        ...
        List<Map<String,Object>> list2 = roleService.selectRoleTitle(id);
        int end=0;
        ...
        if(list2.size()!=0){
            for (int i=0;i<str.length;i++){
                ...
                for(Map<String,Object> M : list2){
                    ...
                    int k = (int)M.get("title_id");
                    num++;
                    if(j==k){
                        break;
                    }
                    if(num == list2.size()){
                        paraMap.put("role_id",id);
                        paraMap.put("title_id",str[i]);
                        end = roleService.insertRoleTitle(paraMap);
                    }
                }
